...
function msgBox(message) -- another generic message box function. I only used it once; that's why it's still here.
  msgboxscript = [[display dialog "]] .. message .. [[" buttons {"ok"} default button "ok" with title "Live Enhancement Suite" with icon POSIX file "/Applications/Live Enhancement Suite.app/Contents/Resources/extensions/hs/les/assets/LESdialog2.icns"]]
...
function buildSettings() -- this function digests the settings.ini file.
  ...
  if i > #settingsArray then break end
    ...
    if string.find(settingsArray[i], "double0todelete =") then
      ...
      msgboxscript = [[display dialog "]] .. _G.double0todelete .. [[" buttons {"ok"} default button "ok" with title "Live Enhancement Suite" with icon POSIX file "/Applications/Live Enhancement Suite.app/Contents/Resources/extensions/hs/les/assets/LESdialog2.icns"]]
...
function requesttime() -- this is the function for when someone checks the current project time. Formatting the seconds into hours/minutes/seconds and presenting it in a nice dialog box.
  ...
  --   b, t, o = hs.osascript.applescript([[tell application "Live Enhancement Suite" to display dialog "Are you sure?" buttons {"Yes", "No"} default button "No" with title "Live Enhancement Suite" with icon POSIX file "/Applications/Live Enhancement Suite.app/Contents/Resources/extensions/hs/les/assets/LESdialog2.icns"]])
  --   if o == [[{ 'bhit':'utxt'("Yes") }]] then
